...
- Agentic SDK: **PydanticAI** (LLM-agnostic — provider swap is a model-string change; OpenAI models for now)
...
- API key: user will provide `OPENAI_API_KEY` via a gitignored `.env`
...
- PydanticAI current API (verified via docs): `Agent("openai:gpt-5.2", output_type=Model, instructions=...)`, `BinaryContent(data, media_type="image/png")` for images, `agent.override(model=TestModel()/FunctionModel(fn))` + `models.ALLOW_MODEL_REQUESTS = False` for offline tests. Docs examples use `openai:gpt-5.2` — use as default model, overridable via flag/env.
...
RunConfig:       model (default "openai:gpt-5.2"), critic_model (defaults to model),
...
**CLI (`cli.py`):** `cad-gen "<spec>" [--max-iterations 5] [--threshold 8] [--model openai:gpt-5.2] [--critic-model M] [--timeout 60] [--out runs]`. Loads `.env`; clear error if `OPENAI_API_KEY` missing. Rich live table: iteration, exec ok/✗, score, top issue. Exit codes: 0 accepted, 1 budget exhausted (best effort still written), 2 hard failure.

## Implementation phases (TDD; superpowers test-driven-development skill)

1. **Scaffold** — `git init`; `uv init --package`; pin 3.13; deps: `pydantic-ai-slim[openai]`, `cadquery`, `trimesh`, `numpy`, `matplotlib`, `typer`, `rich`, `python-dotenv`; dev: `pytest`, `pytest-asyncio`, `ruff`. `.env.example`, `.gitignore`, commit design doc to `docs/superpowers/specs/`. Smoke-check `import cadquery` works (earliest risk retirement).
...
8. **Live E2E** — user pastes key into `.env`; verify model name `openai:gpt-5.2` against the live API at first call (fall back via `--model` if needed); run the verification specs below; tune prompts if the loop doesn't converge.

## Verification

- **Offline (no key):** `uv run pytest` — full suite green, `ALLOW_MODEL_REQUESTS=False` guarantees no accidental API calls.
- **Live E2E (needs `OPENAI_API_KEY` in `.env`):**
...
- OCCT subprocess startup ~2–5s per execution — acceptable; matplotlib z-order artifacts on complex overlaps — acceptable for critique; `gpt-5.2` naming verified against PydanticAI docs but re-confirmed live in Phase 8 with `--model` as escape hatch.
